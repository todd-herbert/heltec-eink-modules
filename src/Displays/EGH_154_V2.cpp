#include "Displays/EGH_154_V2.h"

///Begin the E-Ink library
void EGH_154_V2::begin(const PageProfile profile,  void (*wake_callback)(void), void (*sleep_callback)(void)) {
...
void EGH_154_V2::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void EGH_154_V2::setDefaultColor(uint16_t bgcolor) {
	default_color = bgcolor;
}

void EGH_154_V2::setFlip(Flip flip) {
	this->img_flip = flip;
}
...
void EGH_154_V2::clearPage(uint16_t bgcolor) {
...
bool EGH_154_V2::calculating(bool blocking = true) {
...
void EGH_154_V2::setCursorCorner( char* text, uint16_t x, uint16_t y) {
...
void EGH_154_V2::wait() {
	while(digitalRead(pin_busy) == HIGH)	{	//Low = idle	
		delay(1);
	}
}

void EGH_154_V2::sendCommand(uint8_t command) {
...
void EGH_154_V2::sendData(uint8_t data) {
	SPI.beginTransaction(spi_settings);
...
void EGH_154_V2::update(bool blocking) {
	sendCommand(0x22);
	sendData(0xF7);
	sendCommand(0x20);
...
void EGH_154_V2::writePage() {
	...
	uint8_t end_x = (panel_width / 8) - 1;		//This is taken from the official heltec driver
