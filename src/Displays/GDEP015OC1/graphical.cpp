...
void GDEP015OC1::setFlip(FlipList::Flip flip) {
    this->imgflip = flip;
}
...
void GDEP015OC1::setCursorTopLeft(const char* text, uint16_t x, uint16_t y) {
    int16_t offset_x(0), offset_y(0);
    getTextBounds(text, 0, 0, &offset_x, &offset_y, NULL, NULL);
    setCursor(x - offset_x, y - offset_y);
}

uint16_t GDEP015OC1::getTextWidth(const char* text) {
    int16_t x(0),y(0);
    uint16_t w(0), h(0);
...
uint16_t GDEP015OC1::getTextHeight(const char* text) {
    int16_t x(0),y(0);
    uint16_t w(0), h(0);
    getTextBounds(text, 0, 0, &x, &y, &w, &h);  // Need to keep x and y as they appear to be used internally by getTextBounds()
    return h;
...
uint16_t GDEP015OC1::getTextCenterX(const char* text) {
    int16_t offset_x(0), offset_y(0);
...
uint16_t GDEP015OC1::getTextCenterY(const char* text) {
    int16_t offset_x(0), offset_y(0);
    uint16_t width(0), height(0);
...
uint8_t GDEP015OC1::Bounds::Window::top() {
    return getWindowBounds(T);
}

uint8_t GDEP015OC1::Bounds::Window::right() {
    return getWindowBounds(R);
}

uint8_t GDEP015OC1::Bounds::Window::bottom() {
    return getWindowBounds(B);
}

uint8_t GDEP015OC1::Bounds::Window::left() {
    return getWindowBounds(L);
}

uint8_t GDEP015OC1::Bounds::Window::getWindowBounds(GDEP015OC1::Bounds::Window::side request) {
...
size_t GDEP015OC1::write(uint8_t c) 
{
...
void GDEP015OC1::charBounds(unsigned char c, int16_t *x, int16_t *y, int16_t *minx, int16_t *miny, int16_t *maxx, int16_t *maxy) 
{
