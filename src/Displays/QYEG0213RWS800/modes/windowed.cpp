#include "Displays/QYEG0213RWS800/QYEG0213RWS800.h"

//Methods that run only in windowed mode


bool QYEG0213RWS800::calculating_Windowed() {
...
void QYEG0213RWS800::drawPixel_Windowed(int16_t x, int16_t y, uint16_t color) {
...
void QYEG0213RWS800::writePage_Windowed() {
	//Calculate rotate x start and stop values (y is already done via paging)

	int16_t sx, sy, ex, ey;
	int16_t sy2(0), ey2(0);

	sx = (winrot_left / 8) + 1;	//Screen ignores first byte, handled by "drawing_width"
	sy = page_top;
	ex = (winrot_right / 8);
	ey = page_bottom;
