...
void Heltec_290_V2::begin(const PageProfile profile,  void (*wake_callback)(void), void (*sleep_callback)(void)) {
...
void Heltec_290_V2::grabPageMemory() {
    page_black = new uint8_t[page_bytecount];
}

void Heltec_290_V2::freePageMemory() {
	delete page_black;
}

//Interface directly with display

void Heltec_290_V2::sendCommand(uint8_t command) {
...
void Heltec_290_V2::sendData(uint8_t data) {
	SPI.beginTransaction(spi_settings);
...
void Heltec_290_V2::update(bool blocking) {
	sendCommand(0x21);
	sendData(0x40);
