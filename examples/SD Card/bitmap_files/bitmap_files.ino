...
// DEMO: Bitmaps files  -  draw using .bmp image resources from SD card
// -----------------------------------------------------------------------
...
void setup() {
    ...
    DRAW(display) {
        display.drawMonoBitmapFile(0, snowy_align, "snowy.bmp", BLACK);
    }
    ...
    DRAW(display) {
        display.drawMonoBitmapFile(0, snowy_align, "snowy.bmp", BLACK);
        display.drawMonoBitmapFile(0, tree_align - 50, "lil_tree.bmp", BLACK);  // Lift the new tree 50px from bottom
    ...
    // -------------------------------------
        ...
        // Now that the old boards are gone, we don't need the DRAW loop anymore.
        // We also don't need to redraw the whole scene. Just the new parts.

        display.draw24bitBitmapFile(5, sled_align + 15, "sled.bmp", WHITE);   // Slightly off screen edges, with treating WHITE pixels as transparent
        display.update();
